...
// serde_qs::from_str. All types are inferred by the compiler.
...
{
    deserialize_test_with_config(params, expected, serde_qs::Config::default());
}
...
where
    T: serde::Deserialize<'a> + PartialEq + std::fmt::Debug,
{
    deserialize_test_err_with_config::<T>(params, expected_err, serde_qs::Config::default());
}

#[track_caller]
fn deserialize_test_with_config<'a, T>(params: &'a str, expected: &T, config: serde_qs::Config)
where
    T: serde::Deserialize<'a> + PartialEq + std::fmt::Debug,
{
...
fn querystring_decoding() {
    ...
    let enc_params = serde_qs::to_string(&params).unwrap();
